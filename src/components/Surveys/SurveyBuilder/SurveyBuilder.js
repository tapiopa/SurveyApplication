...
class SurveyBuilder extends Component {
  ...
  componentDidMount() {
    ...
    const newSurveyBuilder =
      this.props.surveyBuilder &&
      this.props.surveyBuilder.survey &&
      this.props.surveyBuilder.survey.newSurvey;
    const newSurvey =
      this.props.survey && this.props.survey.id && this.props.survey.newSurvey;
    const oldSurveyBuilderWithId =
      this.props.surveyBuilder &&
      this.props.surveyBuilder.survey &&
      this.props.surveyBuilder.survey.id &&
      !this.props.surveyBuilder.survey.newSurvey;
    const oldSurveyWithId =
      this.props.survey && this.props.survey.id && !this.props.survey.newSurvey;
    if (newSurveyBuilder) {
      console.log(
        'componentDidMount, new SurveyBuilder',
        this.props.surveyBuilder.survey
      );
      ...
      console.log(
        'componentDidMount with SurveyBuilder, Fetch Survey, id',
        this.props.surveyBuilder.survey.id
      );
      this.props.onFetchSurvey(this.props.surveyBuilder.survey.id);
    } else if (oldSurveyWithId) {
      console.log(
        'componentDidMount with Survey, Fetch Survey, id',
  ...
  componentWillReceiveProps(nextProps, nextContext) {
    ...
    const newSurveyBuilder =
      nextProps.surveyBuilder &&
      nextProps.surveyBuilder.survey &&
      nextProps.surveyBuilder.survey.newSurvey;
    const newSurvey = nextProps.survey && nextProps.survey.id;
    const oldSurveyBuilderWithId =
      nextProps.surveyBuilder &&
      nextProps.surveyBuilder.survey &&
      nextProps.surveyBuilder.survey.id;
    const oldSurveyWithId = nextProps.survey && nextProps.survey.id;
    if (newSurveyBuilder) {
      console.log(
        'componentWillReceiveProps, new SurveyBuilder',
        nextProps.surveyBuilder.survey
      );
      ...
      console.log(
        'componentWillReceiveProps with SurveyBuilder, Fetch Survey, id',
        nextProps.surveyBuilder.survey.id
      );
      this.props.onFetchSurvey(nextProps.surveyBuilder.survey.id);
    } else if (oldSurveyWithId) {
      console.log(
        'componentWillReceiveProps with Survey, Fetch Survey, id',
        nextProps.survey.id
      );
  ...
  updateQuestionString = evt => {
    ...
    // const title = document.getElementById("title").value;
    console.log(
      'updateSurveyTitle, state question input value: ',
  ...
  updateAnswerString = evt => {
    ...
    // const title = document.getElementById("title").value;
    console.log(
      'updateAnswerString, state answer input value: ',
      this.state.inputValue,
      'input id',
      this.state.inputId
    );
    ...
    return (
      <div className={classes.surveyBuilder}>
        ...
        <form className={classes.form}>
          ...
          <FormGroup className={classes.group}>
            <ControlLabel className={classes.label} htmlFor="title">
              Survey Name
            </ControlLabel>
            <FormControl
              className={classes.input}
          ...
          <Button
            onClick={this.saveSurveyIdAndTitle}
            disabled={!this.state.editingSurvey}
            bsStyle="success"
          >
            Save
                  ...
                  if (question.editing) {
                    ...
                    return (
                      <Aux key={question.id} /* show={question.showAnswers}*/>
                        ...
                        {/*question.showAnswers &&*/ question.answers &&
                          question.answers.map(answer => {
                            if (answer.editing) {
                              return (
                                <Aux>
                        ...
                        {!question.showAnswers ? null : (
                          <Aux>
                            ...
                            >
                              ...
                              <td>
                                <Button
                                  onClick={() => this.addNewAnswer(question)}
                                  bsStyle="primary"
                                >
                                  Add Answer
                                </Button>
        ...
        <Button onClick={this.addNewQuestion} bsStyle="success">
          Add a Question
        </Button>

        {/*{!this.state.newSurvey ? null :*/}
        <Button bsStyle="info" onClick={this.goBack}>
          Go Back
        </Button>
        {/*}*/}
        <ButtonToolbar>
          <Button bsStyle="success" bsSize="small" onClick={this.logState}>
            Log State
          </Button>
          <Button bsStyle="success" bsSize="small" onClick={this.logProps}>
            Log Props
          </Button>
        </ButtonToolbar>
      </div>
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    onSaveSurvey: (survey, newSurvey) =>
      dispatch(asyncSaveSurvey(survey, newSurvey)),
    ...
    onSetSurveyTitle: (survey, title) =>
      dispatch(setSurveyTitle(survey, title)),
    onSetQuestionString: (question_id, question_string) =>
      dispatch(setQuestionString(question_id, question_string)),
    onSetAnswerString: (answer_id, answer_string) =>
      dispatch(setAnswerString(answer_id, answer_string))
  };
};
...
export default Protected(
  connect(
    mapStateToProps,
    mapDispatchToProps
  )(withErrorHandler(SurveyBuilder, axios))
);
